...
//    val PRIOR_CORRELATION = 0
    ...
    val sparkConf = new SparkConf().setAppName("cf item-based").setMaster("local")
    ...
    val oriRatings = sc.textFile("/hadoop/SimpleDatasForHuge/ml-20m/smallest").map(line => {
        val fields = line.split("\\s+")
        (fields(0).toLong, fields(1).toLong, fields(2).toInt)
    })
    
   oriRatings.take(10).foreach(println)
    ...
    println("ratings.take(10).toString()");
    ...
    println("item2manyUser.take(10)");
    item2manyUser.collect().foreach(println)
    
    
    val numRatersPerItem = item2manyUser.map(grouped => (grouped._1, grouped._2.size))
    println("numRatersPerItem.take(10)");
    numRatersPerItem.collect().foreach(println)
    ...
    println("ratingsWithSize.take(10)");
   ...
   // ratingsWithSize now contains the following fields: (user, item, rating, numRaters).
    ...
    println("ratings2.take(10)");
    ratings2.collect().foreach(println)
    ...
    println("ratingPairs.take(10)");
    ratingPairs.collect().foreach(println);
    ...
    val tempVectorCalcs =
      ...
      println("tempVectorCalcs.take(10)");
    tempVectorCalcs.collect().foreach(println)
    
    val tempVectorCalcs2 = tempVectorCalcs.groupByKey();
           println("tempVectorCalcs2.take(10)");
       tempVectorCalcs2.collect().foreach(println)
    
    val vectorCalcs = tempVectorCalcs2.map(data => {
        val key = data._1
        val vals = data._2
        val size = vals.size
      ...
      //.filter(x=>x._2._1>1)
       ...
       println("vectorCalcs.take(10)");
       vectorCalcs.collect().foreach(println)
    ...
    val vectorCalcsTotal = vectorCalcs ++ inverseVectorCalcs
     ...
     println("inverseVectorCalcs.take(10)");
       inverseVectorCalcs.collect().foreach(println)
    ...
    val tempSimilarities =
      vectorCalcsTotal.map(fields => {
       ...
       println("tempSimilarities.take(10)");
       tempSimilarities.collect().foreach(println)
